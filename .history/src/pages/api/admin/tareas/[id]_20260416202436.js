...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'PUT') {
      ...
      // Si supervisor, validar que sea de su planta
      if (verify.rol === 'supervisor' && currentTask.planta_id !== verify.planta_id) {
        return res.status(403).json({
          error: 'Forbidden',
          detail: 'No tienes acceso a esta tarea',
        });
      ...
      if (supervisado_por !== undefined) updateData.supervisado_por = supervisado_por;
      ...
      if (porcentaje_avance !== undefined) updateData.porcentaje_avance = porcentaje_avance;
