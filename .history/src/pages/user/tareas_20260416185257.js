  ...
  const cargarTareas = async () => {
    try {
      const { data: tareasData, error: err } = await supabase
        .from('tareas')
        .select(
          '*, prioridad:prioridades(nombre), creado_por_user:usuarios!creado_por(nombre_completo)'
        )
      ...
      <div className={styles.contenedor}>
        <div className={styles.encabezado}>
          <div>
            ...
            <p className={styles.contador}>{tareasFiltradas.length} de {tareas.length} tareas</p>
          </div>
          <select
