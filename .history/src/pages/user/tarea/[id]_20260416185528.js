...
export default function TareaDetalle() {
  ...
  const cargarTarea = async () => {
    try {
      const { data: tareaData, error: err } = await supabase
        .from('tareas')
        .select(
          `
          *,
          prioridad:prioridades(nombre),
          creado_por_user:usuarios!creado_por(nombre_completo),
          asignado_user:usuarios!asignado_a(nombre_completo),
          planta:plantas(nombre, pais:paises(nombre))
        `
        )
        .eq('id', id)
      ...
      setTarea(tareaData);
      setNuevoEstado(tareaData.estado);
  ...
  const cargarComentarios = async () => {
    try {
      const { data: comentariosData, error: err } = await supabase
          ...
          <section className={styles.seccion}>
            ...
            <div className={styles.gridInfo}>
              <div className={styles.item}>
                ...
                <span className={styles[`prioridad-${tarea.prioridad?.nombre}`]}>
              ...
              <div className={styles.item}>
                ...
                <span className={new Date(tarea.fecha_limite) < new Date() ? styles.vencido : ''}>
