  ...
  const cargarStats = async () => {
    try {
      const { data: tareas, error: err } = await supabase
        .from('tareas')
        .select('id, estado:estados_tarea(nombre), porcentaje_avance')
      ...
      if (montadoRef.current) {
        const stats = {
          ...
          tareasCompletas: tareas.filter((t) => t.estado?.nombre === 'completado')
            .length,
          tareasEnProceso: tareas.filter((t) => t.estado?.nombre === 'en_proceso')
            .length,
          avancePromedio:
            tareas.length > 0
              ? Math.round(
