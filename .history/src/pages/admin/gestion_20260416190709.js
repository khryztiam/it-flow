...
export default function GestionAdmin() {
  ...
  useEffect(() => {
    if (!cargandoAuth) {
      // Cargar plantas disponibles siempre (necesarias para los renders)
      const cargarPlantasBase = async () => {
        try {
          const paisesData = await obtenerPaises();
          setPaises(paisesData);
          if (paisesData.length > 0) {
            const plantasData = await obtenerPlantasPorPais(paisesData[0].id);
            setPlantas(plantasData);
          }
        } catch (e) {
          console.error('Error cargando plantas base:', e);
        }
      };
