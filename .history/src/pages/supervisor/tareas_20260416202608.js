...
export default function TareasSupervisor() {
  ...
  const cargarDatos = async () => {
    try {
      ...
      const [prioridadesRes, estadosRes, tareasRes, usuariosRes] = await Promise.all([
      ...
      setTareas((tareasRes || []).filter(t => t.planta_id === usuarioDetalles.planta_id));
  ...
  const handleGuardar = async () => {
    ...
    try {
      if (!valores.titulo || !valores.fecha_limite || !valores.prioridad_id || !valores.estado_id) {
