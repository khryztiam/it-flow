...
export default function TareaDetalleSupervisor() {
  ...
  const cargarTarea = async () => {
    try {
      setCargando(true);
      setError('');
      const tareaData = await callAPI(`/api/supervisor/tareas/${id}`);

      if (!tareaData) {
        router.push('/supervisor/tareas');
        return;
      }
      ...
      setNuevoAsignado(tareaData.asignado_a || '');
    } catch (err) {
  ...
  const agregarComentario = async () => {
    ...
    try {
      ...
      const result = await callAPI(`/api/supervisor/tareas/${id}`, 'POST', {
  ...
  const cargarEvidencias = async () => {
    try {
      const data = await callAPI(`/api/supervisor/tareas/${id}/upload`);
      setEvidencias(data || []);
    } catch (err) {
  ...
  const eliminarEvidencia = async (evidenciaId) => {
    ...
    try {
      setError('');
      await callAPI(`/api/supervisor/tareas/${id}/upload`, 'DELETE', {
        evidenciaId,
      });
  ...
  const actualizarTarea = async () => {
    try {
      ...
      if (nuevoAsignado && nuevoAsignado !== tarea.asignado_a) {
        payload.asignado_a = nuevoAsignado;
      }

      await callAPI(`/api/supervisor/tareas/${id}`, 'PUT', payload);
