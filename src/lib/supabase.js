...
export const supabase = createClient(supabaseUrl, supabaseAnonKey, {
  auth: {
    ...
    storage: {
      // Verificar que estamos en cliente antes de acceder a localStorage (SSR-safe)
      getItem: (key) =>
...
// Obtener TODAS las plantas (sin filtro por país)
export const obtenerTodasLasPlantas = async () => {
  const { data, error } = await supabase
    .from('plantas')
    .select(
      `
